...
namespace Flock{
	...
	public class FlockManipulator_CentroidLerped : FlockManipulator {
		...
		void controllerSpeed(){
			...
			speed = speed * 1000;
			speed += Vector3.Distance (attractor.transform.position, previousPos) * distanceMultiplier;
			speed /= 1001f+(prevSpeed*prevSpeed*3f);
            //speed = Mathf.Min(1.0f, speed);
			previousPos = new Vector3(
				attractor.transform.position.x,
				attractor.transform.position.y,
				attractor.transform.position.z);
		}
...
//				Debug.Log (targetLerp[i]);
				...
				Vector3 newPos2 = attractor.transform.localToWorldMatrix.MultiplyVector (targets2 [i]) + attractor.transform.position;
