...
public class MixedIKManager : MonoBehaviour {
	...
	*/
	...
	void solveBounds()
	{
		xmin = float.MaxValue; ymin = float.MaxValue; zmin = float.MaxValue; xmax = float.MinValue; ymax = float.MinValue; zmax = float.MinValue;
		
		Vector3[] ts = new Vector3[] { this.mocapHeadset.position, this.mocapRightWrist.position, this.mocapLeftWrist.position, this.mocapRightAnkle.position, this.mocapLeftAnkle.position };
		
		for (int i = 0; i < ts.Length; i++)
		{
			if (ts[i].x < xmin) xmin = ts[i].x;
			if (ts[i].y < ymin) ymin = ts[i].y;
			if (ts[i].z < zmin) zmin = ts[i].z;
			if (ts[i].x > xmax) xmax = ts[i].x;
			if (ts[i].y > ymax) ymax = ts[i].y;
			if (ts[i].z > zmax) zmax = ts[i].z;
		}
		
		maxBound = new Vector3(xmax, ymax, zmax);
		minBound = new Vector3(xmin, ymin, zmin);
		centerBound = (maxBound + minBound) / 2f;
		extents = maxBound - centerBound;

		skinnedRenderer.bounds
		skinnedRenderer.bounds.center = centerBound;
		skinnedRenderer.bounds.extents = extents;
	}
*/
